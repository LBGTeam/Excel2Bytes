...
def FieldExcelScript(scriptName, secValueType, bytesName, scriptBuilder, extraNamespace):
    ...
    Scripts.AppendField('m_Cached', f'Dictionary<uint, {GetCShapeType(secValueType)}>', 'private')
    Scripts.AppendField('m_Entity', f'Dictionary<uint, {OfferMap}>', 'private')
    ...
    Scripts.AppendLine(f'm_Cached = new Dictionary<uint, {GetCShapeType(secValueType)}>();')
    Scripts.AppendLine(f'm_Entity = new Dictionary<uint, {OfferMap}>();')
    ...
    Scripts.AppendLine(f'reader = new BinaryReader({Config.ResLoadScripts().format(f"{bytesName}.bytes")},'
                       f' Encoding.UTF8);')
    Scripts.AppendLine(f'reader.BaseStream.Position = 0;')
    Scripts.AppendLine('uint index = 0;')
    Scripts.BeginWhile('true')
    Scripts.AppendLine(f'm_Entity.Add(index, ({OfferMap})reader.BaseStream.Position);')
    GetFieldInitDataSize(secValueType, Scripts)
    Scripts.AppendLine('//判断是否读取完毕')
    Scripts.BeginIf('reader.BaseStream.Position >= reader.BaseStream.Length')
    Scripts.AppendLine('break;')
    Scripts.EndIf()
    Scripts.AppendLine('index++;')
    Scripts.EndWhile()
    Scripts.EndMethod()
    Scripts.AppendEmptyLine()
    ...
    Scripts.EndMethod()
    Scripts.AppendEmptyLine()
    Scripts.BeginMethod('ReadData', 'public', GetCShapeType(secValueType), f'uint id')
    Scripts.BeginIf(f'm_Cached.TryGetValue(id, out var value)')
    Scripts.AppendLine(f'return value;')
    Scripts.EndIf()
    Scripts.BeginIf(f'm_Entity.TryGetValue(id, out var offer)')
    Scripts.AppendLine('reader.BaseStream.Position = (long)offer;')
    GetDataAssignment(secValueType, 'value', Scripts)
    Scripts.AppendLine(f'm_Cached.Add(id, value);')
    Scripts.AppendLine(f'return value;')
    Scripts.EndIf()
    Scripts.AppendLine('return default;')
    Scripts.EndMethod()
    Scripts.AppendEmptyLine()
    Scripts.AppendEnter()
